    ...
    for_each_l(
        ...
        [&r, &req, &res](auto &item) {
          ...
          constexpr bool has_befor_mtd =
              has_before<decltype(item), request &, response &>::value || has_before_pointer<decltype(item), request&, response&>::value;
          constexpr bool is_pointer = std::is_pointer<std::decay_t<decltype(item)>>::value;
    ...
    for_each_r(
        ...
        [&r, &req, &res](auto &item) {
          ...
          constexpr bool has_after_mtd =
              has_after<decltype(item), request &, response &>::value || has_after_pointer<decltype(item), request&, response&>::value;
          constexpr bool is_pointer = std::is_pointer< std::decay_t<decltype(item)>>::value;
    ...
    for_each_r(
        ...
        [&r, result = std::move(result), &req, &res](auto &item) {
          ...
          if constexpr (has_after<decltype(item), T, request&, response&>::value || 
              has_after_pointer<decltype(item), request&, response&>::value) {
              constexpr bool is_pointer = std::is_pointer< std::decay_t<decltype(item)>>::value;
              if constexpr (is_pointer)
