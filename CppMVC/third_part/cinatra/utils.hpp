...
template <class T, typename... Args>
using has_before_pointer_t = decltype(std::declval<T>()->before(std::declval<Args>()...));

template <class T, typename... Args>
using has_after_t = decltype(std::declval<T>().after(std::declval<Args>()...));

template <class T, typename... Args>
using has_after_pointer_t = decltype(std::declval<T>()->after(std::declval<Args>()...));
}  // namespace
...
template <typename T, typename... Args>
using has_before_pointer = is_detected<has_before_pointer_t, T, Args...>;

template <typename T, typename... Args>
using has_after = is_detected<has_after_t, T, Args...>;

template <typename T, typename... Args>
using has_after_pointer = is_detected<has_after_pointer_t, T, Args...>;
